...
fun runStep(input :String) { when(input){
    "" -> {}
    "login" -> {
    ...
    "signup" -> {
        ...
        val status = socket.requestResponse(SignUpDataBlob(
            clientID = clientID,
    ...
    "money" -> {
        ...
        val money = socket.requestResponse(StringDataBlob(
            clientID = clientID,
    ...
    miner@ while(true) {
        ...
        when (val inn = scan.nextLine()) {
            ...
            "status" ->{
                ...
                println("Percent accepted:${m.mined.toDouble().div(m.failed.toDouble().plus(m.mined.toDouble())).times(100)}%")
            }
            "back","exit" -> {
                return
            }
